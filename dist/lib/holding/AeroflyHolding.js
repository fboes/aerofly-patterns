...
export class AeroflyHolding {
    static async init(configuration) {
        ...
        for (const date of dates) {
            try {
                const scenario = await Scenario.init(self.holdingFix, self.configuration, self.aircraft, date, index++, initialBearing);
...
`;
    ...
    buildGeoJson() {
        ...
        geoJson.addFeature(new Feature(this.holdingFix.position, {
            id: 0,
        ...
        this.scenarios.forEach((scenario, index) => {
            geoJson.addFeature(new Feature(new LineString([
                ...
                ...scenario.mission.checkpoints.map((cp) => {
            ...
            geoJson.addFeature(new Feature(new Point(scenario.mission.origin.longitude, scenario.mission.origin.latitude, scenario.mission.origin.alt), {
