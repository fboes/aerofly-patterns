...
export class FileWriter {
    static async writeFile(app, saveDirectory) {
        ...
        if (app.configuration.directoryMode) {
            promises.push(fs.writeFile(`${saveDirectory}/README.md`, app.buildMarkdown()), fs.cp(app.configuration.geoJsonFile, `${saveDirectory}/${app.configuration.environmentId}.geojson`));
        }
        ...
        if (!app.configuration.doNotGeneratePois) {
            ...
            promises.push(fs.writeFile(`${poiDirectory}/${poi_id}.toc`, app.buildEmergencySitesToc()), fs.writeFile(`${poiDirectory}/${poi_id}.tsl`, app.buildEmergencySitesTsl()), fs.cp(path.join(path.dirname(fileURLToPath(import.meta.url)), "../../../src/data/hems/fallback"), poiFallbackDirectory, { recursive: true }));
        }
