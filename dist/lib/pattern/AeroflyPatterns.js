...
export class AeroflyPatterns {
    ...
    buildGeoJson() {
        ...
        this.airport.runways.forEach((r) => {
            geoJson.addFeature(new Feature(r.position, {
                title: r.id,
                "marker-symbol": r === scenario.activeRunway ? "triangle" : r.runwayType === "H" ? "heliport" : "triangle-stroked",
        ...
        this.airport.navaids.forEach((n) => {
            geoJson.addFeature(new Feature(n.position, {
                title: n.id,
        ...
        geoJson.addFeature(new Feature(scenario.aircraft.position, {
        ...
        geoJson.addFeature(new Feature(new LineString(waypoints), {
        ...
        scenario.patternWaypoints.forEach((p) => {
            geoJson.addFeature(new Feature(p.position, {
                title: p.id,
    ...
    buildCustomMissionTmc() {
        ...
        const missions = this.scenarios.map((s, index) => {
            ...
            if (this.configuration.noGuides) {
                ...
                mission.finish = new AeroflyMissionTargetPlane(targetPosition.longitude, targetPosition.latitude, s.aircraft.heading);
            }
            return mission;
        });
